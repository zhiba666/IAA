  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname.startsWith('/__acceptance/capture/')) {
        ...
        await writeFile(path.join(OUTPUT, 'request-report.json'), JSON.stringify({ scope: fixtures.scope, requests }, null, 2) + '\n');
        send(201, JSON.stringify({ saved: path.join(OUTPUT, name), bytes: count }), types['.json']); return;
      }
      ...
      if (target === path.join(WEB, 'index.html')) {
        const name = url.searchParams.get('fixture') || 'fresh-shortage';
        const fixture = fixtureMap[name];
        if (!fixture) { send(400, 'Unknown fixture; use ' + Object.keys(fixtureMap).join(', ')); return; }
        const input = { fixture, scope: fixtures.scope, storageKey: fixtures.storageKey, port:PORT,
