...
class AudioEngine {
  ...
  play(name) {
    ...
    if (name === 'pop' || name === 'tap' || name === 'click') {
      ...
      for (let index = 0; index < 7; index++) this.tone(300 + index * 80, 100, 0.12, 0.055, 'triangle', index * 0.035);
      this.tone(190, 85, 0.28, 0.08, 'sine', 0);
    } else if (name === 'upgrade' || name === 'unlock' || name === 'machine') {
  ...
  playNative(name) {
    ...
    const key = aliases[name] || (['pop','burst','upgrade','order','complete','error'].includes(name)?name:'click');
