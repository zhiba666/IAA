...
class GameInterface {
  ...
  home(v,ui){
    ...
    const dockH=this.firstGeneration?(selected?(collapsed?52:this.nativeCompact?120:short?148:176):(this.nativeCompact?120:short?142:158)):(selected?(collapsed?52:short?160:202):(short?158:174));
  ...
  artHud(v,ui,headerH){
    ...
    const sample=v.state.playedSeconds<10?'实测 · 开工不足10秒':updating?'近10秒均速 · 更新中':'最近10秒均速';
    ...
    if(!short)this.label('小小爆米花厂 · 第 1 代',16,top+61,w-32,15,C.ink,800);
    ...
    const hint=failed?'美术加载失败 '+failed+' 项':pending?'正在装配首代美术…':ui.toast||(v.insights?.bottleneck?.label||'观察供料与积压，寻找生产限制');
  ...
  station(v,ui,y,height){
    ...
    if(ui.stationDetails){
      ...
      const detail=this.nativeCompact&&up?(up.lineImproves?'供料与库存稳定后体现，实测见顶部':'暂不提高稳定出货，为后续改造预留能力'):'实测通过 '+rate(s.actualRate)+' 份/秒（近10秒）';
      ...
      this.label((up.lineRequiresExpansion?'扩建后预计出货 ≈ ':'预计稳定出货 ≈ ')+rate(up.lineBefore)+'→'+rate(up.lineAfter)+' 份/秒',x+12,contentY+lineGap,w-24,this.nativeCompact?11:13,C.green,750);
