...
public class SixGenAlphaExport {
 public static object Export(string source,string target,int limit) {
  using(var input=new Bitmap(source)) {
   ...
   return new {sourceSize=new[]{input.Width,input.Height},sourceRect=new[]{crop.X,crop.Y,crop.Width,crop.Height},exportSize=new[]{width,height},uniformScale=scale,contentOffset=new[]{gutter,gutter},alphaMin=min,alphaMax=max,alphaZero=zero,alphaSemi=semi,hasRealAlpha=zero>0&&max>0,format="PNG/RGBA",cropPolicy="alpha>0 bounding box plus five source pixels; complete source preserved",resize="uniform HighQualityBicubic; rounded raster dimensions; UI adds two-pixel transparent gutter"};
  }
 }
}
'@
