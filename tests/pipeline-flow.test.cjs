...
test('insufficient coins and unknown legacy inputs cannot alter stock or money', () => {
  ...
  h.click('station:cup'); h.click('upgrade:cup');
  assert.deepEqual(economy(h.snapshot()), economy(before));
  h.click('close');
  for (const action of ['tap','start','claimOrder','releasePressure','ad:turbo','ad:offline','ad:brand','adComplete','simulate:complete','quest:0','energy','claimOffline','upgrade:auto','upgrade:tap','upgrade:value','brand','order','modules']) h.click(action);
  assert.deepEqual(economy(h.snapshot()), economy(before));
  assert.equal(h.legacyCalls(), 0);
});
...
test('save reload preserves partial work, purchased capacity, settings and one-time intro dismissal', () => {
  const h = harness({ save: progressed() });
  h.click('station:cup'); h.click('upgrade:cup'); h.frame(123);
...
test('cancelled, dragged and interrupted pointers cannot buy a station upgrade', () => {
  ...
  h.pointer('down','upgrade:cup'); h.pointer('cancel','upgrade:cup');
  h.pointer('down','upgrade:cup'); h.pointer('move','upgrade:cup',1,150,100); h.pointer('up','upgrade:cup');
  h.pointer('down','upgrade:cup'); h.hide(); h.show(); h.pointer('up','upgrade:cup');
  assert.deepEqual(economy(h.snapshot()), before);
});
