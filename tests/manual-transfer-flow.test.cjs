...
test('P0 real entry requires continuous drag and settles only actual sales', () => {
  const h = setup();
  ...
  assert.equal(h.ui().transfer, null, 'separate taps never reserve or commit');
...
test('P0 autosave does not interrupt held cargo and lifecycle save restores no reservation', () => {
  ...
  loaded.drag('transfer:source', 'transfer:target'); loaded.run(3);
...
test('resize clears abandoned touch IDs so a new finger can resume without an old up event', () => {
  ...
  h.resize();
  h.drag('transfer:source', 'transfer:target', 2);
  h.run(3);
  assert.equal(h.snapshot().state.totalSold, 4);
  conserved(h);
});
test('P0 storage failure keeps transferred stock playable and restart retains experimental mode', () => {
  ...
  h.drag('transfer:source', 'transfer:target');
