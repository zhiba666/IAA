...
function boot({ gameGlobalOnly = false, raf = true, browserShims = false, safeTop = 47, menuApi = true } = {}) {
  ...
  function load(filename) {
    ...
    if(filename===path.join(PACKAGE,'game.bundle.js')) {
      ...
      const marker='const renderer = new Renderer(ctx);';
  ...
  const h = {
    ...
    clickText(label) {
      ...
      const entry=matches[matches.length-1];this.click(entry.x,entry.y);
    },
    verifyLayout() {
...
test('generated Douyin entry: native host selection, safe-area input, audio, lifecycle and bounded scheduling', async t => {
  for (const mode of [
    ...
    assert.ok(!h.texts.includes('⚙'),'settings belong to the factory sheet');
    ...
    h.clickText(/^点机器/); h.events.onHide();
    ...
    h.clickText(/^点机器/); h.events.onHide(); assert.equal(h.saved().taps, 2);
    ...
    h.clickText(/^点机器/);h.events.onHide();assert.equal(h.saved().taps,3,'failed native ads must preserve production input');
    h.events.onShow({});h.frame();
    h.info.windowHeight = 780; h.info.safeArea.bottom = 746;
    h.events.onWindowResize({}); h.frame(); h.verifyLayout();
    h.clickText(/^点机器/);h.events.onHide();assert.equal(h.saved().taps,4,'resized safe-area coordinates must still reach production');
  });
});
...
test('native scene fills the top while controls avoid device insets and the menu capsule', async t => {
  for(const scenario of [
    ...
    assert.ok(h.texts.includes('订单装车'),'the menu-clearing order entry remains clickable');
    h.clickText('×');h.frame();
    ...
    for(const [index,event] of ['onWindowResize','onShow'].entries()) {
      ...
      h.clickText(/^本单 /);h.frame();h.verifyControls();
      assert.ok(h.texts.includes('订单装车'),'updated menu coordinates must preserve order input');
      h.clickText('×');h.frame();h.clickText(/^点机器/);h.events.onHide();
